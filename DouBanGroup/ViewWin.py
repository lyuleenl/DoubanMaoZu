...
secondText=tk.StringVar(value='金素')
thirdText=tk.StringVar(value='素立高')
...
thirdKey= tk.Entry(window,textvariable=thirdText,show=None, font=('Arial', 14))
...
tk.Label(window, text='二级筛选字符1: ').place(x=50,y=95)

#Button组件
b = tk.Button(window, text="Start", command=lambda:[thread_it(ViewFun.Start,urlText.get(),firstText.get(),secondText.get(),thirdKey.get()),thread_it(showMsg)])
b.pack()
#addAttach=tk.Button(window,text="添加筛选",command=lambda :[thread_it(addAttachKey)])
#addAttach.pack()
...
def addAttachKey():
    text = tk.StringVar()
    ui=tk.Entry(window, textvariable=text,show=None, font=('Arial', 14))  # 显示成明文形式
    ui.pack(ipadx="10")
    global high
    high+=20
    pix = '600x'+str(high)
    tk.Label(window, text='二级筛选字符1: ').place(x=50, y=ui)
    window.geometry(pix)
